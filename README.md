...
启用内置的 `account` 客户端，确保普通用户拥有其 `manage-account` 角色，并允许该角色进入门户客户端签发的 access token。Token 的 audience 需要包含 `account`，可在门户客户端添加 Audience mapper，Included Client Audience 设为 `account`，并开启 Add to access token。
...
修改密码、删除认证器等操作会跳转到 Keycloak 完成，然后返回门户。
...
帮助页的“重置密码”和账户安全页的“忘记密码？”均通过 `/auth/reset-password` 进入 Keycloak 的 `protocol/openid-connect/forgot-credentials` 重置流程，复用门户的 OIDC 回调，完成后返回账户安全页。
